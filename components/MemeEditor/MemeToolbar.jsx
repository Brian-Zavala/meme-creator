          ...
          <div className={`flex flex-col justify-center transition-opacity duration-300 ${isCollapsed ? 'opacity-0' : 'opacity-100 px-6 py-6 min-h-[80px]'}`}>
            ...
            {activeTab === "text" && (
              <div id="text-tools-panel" role="tabpanel" className="flex flex-col items-center justify-start w-full gap-6">
                ...
                {/* Collapsible Text Styling Section - Mobile First */}
                {(hasText || hasStickers) && (
                  <>
                    ...
                    <button
                      onClick={() => {
                        if (navigator.vibrate) navigator.vibrate(10);
                        startTransition(() => setShowSliders(!showSliders));
                      }}
                      className="lg:hidden flex items-center justify-center gap-2 w-full py-2 px-4 rounded-lg bg-slate-800/50 border border-slate-700 text-slate-400 hover:text-white hover:border-slate-500 transition-all active:scale-[0.98]"
                    >
                      <Settings2 className="w-4 h-4" />
                      <span className="text-xs font-bold uppercase tracking-wider">
                        {showSliders ? 'Hide' : 'Show'} Text Styling
                      </span>
                      <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${showSliders ? 'rotate-180' : ''}`} />
                    </button>

                    {/* Collapsible Container */}
                    <div className={`grid transition-[grid-template-rows] duration-300 ease-out ${showSliders ? 'grid-rows-[1fr]' : 'grid-rows-[0fr] lg:grid-rows-[1fr]'}`}>
                      <div className="overflow-hidden">
                        <div className={`flex flex-col gap-6 transition-opacity duration-300 ${showSliders ? 'opacity-100 pt-2' : 'opacity-0 lg:opacity-100 lg:pt-2'}`}>
