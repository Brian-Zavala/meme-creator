  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = async () => {
      ...
      try {
        ...
        // --- CHUNKED PROCESSING: Prevents UI lockup ---
        const CHUNK_SIZE = 50000; // Process 50k pixels per chunk
        const totalPixels = data.length / 4;

        const processChunk = (startPixel, endPixel) => {
        ...
        // Process in chunks, yielding to browser between each
        ...
        for (let start = 0; start < totalPixels; start += CHUNK_SIZE) {
          ...
          // Yield to browser every chunk to prevent lockup
          if (end < totalPixels) await yieldToMain();
        }
